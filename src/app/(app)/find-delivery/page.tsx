...
export default function FindDeliveryPage() {
  const deliveryRides = useMemo(() => rides.filter(ride => ride.features.parcelSpace !== 'None'), []);
  ...
  return (
    <div className="max-w-4xl mx-auto py-8 px-4">
         ...
         <div className="flex items-center justify-between">
            <div className="flex gap-4 items-center">
             ...
             <div className="flex gap-4 items-center text-sm text-muted-foreground">
                 ...
                 <span className="flex items-center gap-2"><Package className="h-4 w-4" />1 Parcel</span>
             </div>
      ...
      <div className="space-y-6">
        {deliveryRides.length > 0 ? deliveryRides.map(ride => (
          <DeliveryCard key={ride.id} ride={ride} />
        )) : (
            <Card>
                <CardContent className="p-12 text-center text-muted-foreground">
                    <p>No available deliveries match your search.</p>
                </CardContent>
            </Card>
        )}
      </div>
